...
{
    soc_reg_t reg;
    uint32 schan_override;
    int rv;

    schan_override = SOC_CONTROL(unit)->schan_override;
    SOC_CONTROL(unit)->schan_override = 1;

    reg = egress ? EGR_SBS_CONTROLr : SBS_CONTROLr;
    rv = soc_reg_field32_modify(unit, reg, REG_PORT_ANY, PIPE_SELECTf, pipe);

    SOC_CONTROL(unit)->schan_override = schan_override;
...
soc_trident_mem_config(int unit)
{
    ...
    switch (dev_id) {
        ...
        case BCM56847_DEVICE_ID:
            ...
            sop->memState[L3_DEFIP_Xm].index_max = 4095;
            sop->memState[L3_DEFIP_Ym].index_max = 4095;
