...
#ifdef BCM_TRIDENT2_SUPPORT
    if (ctr_reg == SOC_COUNTER_NON_DMA_MMU_QCN_CNM && ar_idx == -1 &&
        ...
        if (non_dma->flags & _SOC_COUNTER_NON_DMA_VALID) {
            ...
            for (i = 0; i < non_dma->num_entries; i++) {
                soc_mem_field_set(unit, non_dma->mem,
                                  &non_dma->dma_buf[0][entry_words * i],
                                  non_dma->field, fval);
                COUNTER_ATOMIC_BEGIN(s);
                soc->counter_sw_val[port_index + i] = val64;
                soc->counter_hw_val[port_index + i] = val64;
                COMPILER_64_SET(soc->counter_delta[port_index + i], 0, 0);
                COUNTER_ATOMIC_END(s);
            }
            for (i = 0; i < 4; i++) {
                if (non_dma->flags & _SOC_COUNTER_NON_DMA_DO_DMA) {
                    ...
                    rv = soc_mem_write_range(unit, non_dma->dma_mem[i],
                                             MEM_BLOCK_ANY,
                                             non_dma->dma_index_min[i],
                                             non_dma->dma_index_max[i],
                                             non_dma->dma_buf[i]);
