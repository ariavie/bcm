...
#endif
#define _ERR_MSG_MODULE_NAME BSL_LS_SOC_INTR
...
exit:
    if (SOC_FAILURE(_rv)) {
        LOG_ERROR(BSL_LS_SOC_INTR,
                  (BSL_META_U(unit,
                              "Internal error in soc_cmn_block_error")));
    }
    LOG_VERBOSE(BSL_LS_SOC_INTR,
                (BSL_META_U(unit,
                            "exit \n")));
...
exit:
    if (SOC_FAILURE(_rv)) {
        LOG_ERROR(BSL_LS_SOC_INTR,
                  (BSL_META_U(unit,
                              "Internal error in soc_cmn_error")));
    }
    LOG_VERBOSE(BSL_LS_SOC_INTR,
                (BSL_META_U(unit,
                            "exit \n")));
...
soc_active_interrupts_get(int unit, int flags, int max_interrupts_size, soc_interrupt_cause_t *interrupts, int *total_interrupts)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_INTR,
                (BSL_META_U(unit,
                            "%s(): cmic_irq_stat[0]=0x%x, cmic_irq_stat[1]=0x%x,\n"),
...
int soc_interrupt_flags_set(int unit, int interrupt_id, uint32 flags)
{
    ...
    if(SOC_IS_DFE(unit)) {
        ...
        _SOC_IF_ERR_EXIT(SOC_DFE_WARM_BOOT_ARR_VAR_SET(unit, INTR_FLAGS, interrupt_id, &flags));
...
int soc_interrupt_flags_get(int unit, int interrupt_id, uint32* flags)
{
    ...
    if(SOC_IS_DFE(unit)) {
        ...
        if(interrupt_id < 0 || interrupt_id >= nof_interrupts) {
         ...
         _SOC_IF_ERR_EXIT(SOC_DFE_WARM_BOOT_ARR_VAR_GET(unit, INTR_FLAGS, interrupt_id, flags));
...
int soc_interrupt_storm_timed_period_set(int unit, int interrupt_id, uint32 storm_timed_period)
{
    ...
    if(SOC_IS_DFE(unit)) {
        ...
        if(interrupt_id < 0 || interrupt_id >= nof_interrupts) {
         ...
         _SOC_IF_ERR_EXIT(SOC_DFE_WARM_BOOT_ARR_VAR_SET(unit, INTR_STORM_TIMED_PERIOD, interrupt_id, &storm_timed_period));
...
int soc_interrupt_storm_timed_period_get(int unit, int interrupt_id, uint32* storm_timed_period)
{
    ...
    if(SOC_IS_DFE(unit)) {
        ...
        _SOC_IF_ERR_EXIT(SOC_DFE_WARM_BOOT_ARR_VAR_GET(unit, INTR_STORM_TIMED_PERIOD, interrupt_id, storm_timed_period));
...
int soc_interrupt_storm_timed_count_set(int unit, int interrupt_id, uint32 storm_timed_count)
{
    ...
    if(SOC_IS_DFE(unit)) {
        ...
        _SOC_IF_ERR_EXIT(SOC_DFE_WARM_BOOT_ARR_VAR_SET(unit, INTR_STORM_TIMED_COUNT, interrupt_id, &storm_timed_count));
...
int soc_interrupt_storm_timed_count_get(int unit, int interrupt_id, uint32* storm_timed_count)
{
    ...
    if(SOC_IS_DFE(unit)) {
        ...
        _SOC_IF_ERR_EXIT(SOC_DFE_WARM_BOOT_ARR_VAR_GET(unit, INTR_STORM_TIMED_COUNT, interrupt_id, storm_timed_count));
