...
int _bcm_kt2_port_lanes_set(int unit, bcm_port_t port, int lanes)
{
    ...
    char               config_str[80];
    uint8              lane_incr[KT2_MAX_MXQPORTS_PER_BLOCK]={4,2,0,1};
 ...
 for (try_loop = 0; try_loop < 2; try_loop++) { 
    ...
    if ((mxqblock == 8) || (mxqblock == 9)) {
         ...
         case 0:
               ...
               for (loop = 0;
                    loop < KT2_MAX_MXQPORTS_PER_BLOCK;
                    loop += lane_incr[new_lanes-1] ) {
                    mxqblock_port=(*kt2_mxqblock_ports[unit])[mxqblock][loop];
                    portgroup = soc_property_port_get(unit, mxqblock_port, 
                                                      spn_PORTGROUP, 0);
                    if (portgroup == 0) {
         ...
         case 1:
               ...
               bsl_warn(BSL_BCM_PORT, unit,
                        "Generated Portgroup setting as %s=4  for port:%d \n",
                        config_str, port);
               break;
         case 2:
               for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop+=2) {
                    ...
                    bsl_warn(BSL_BCM_PORT, unit,
                            "Generated Portgroup setting as %s=2 for port:%d\n",
                            config_str, mxqblock_port);
               }
               break;
         case 4:
               for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
                    ...
                    bsl_warn(BSL_BCM_PORT, unit,
                            "Generated Portgroup setting as %s=1 for port:%d\n",
                            config_str, mxqblock_port);
               }
               break;
         }
