...
int bcm_esw_ces_crm_dma_msg_send_receive(int unit, uint8 t_subclass, uint8 *data, 
	...
	LOG_CLI((BSL_META_U(unit,
                            "%s: Received message type error or missmatch, "
                            "mclass:%d  subclass:%d\n"),
...
AgResult bcm_esw_ces_pbi_cb(AG_U32 n_channel_id, AG_U32 n_user_data) {
    ...
    LOG_WARN(BSL_LS_BCM_CES,
             (BSL_META_U(unit,
                         "%s:PBFoverflowonservice:%lu\n"), FUNCTION_NAME(), n_channel_id));
...
AgResult bcm_esw_ces_cwi_cb(AG_U32 n_channel_id, AG_U32 n_user_data, AG_U16 n_cw) {
    ...
    LOG_VERBOSE(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit,
                            "%s:Service:%luCEScontrolwordbitschangedto:0x%04x\n"),
                 FUNCTION_NAME(), n_channel_id, n_cw));
...
AgResult bcm_esw_ces_psi_cb(AG_U32 n_channel_id, AG_U32 n_user_data, AgNdSyncType n_sync) {
    ...
    LOG_VERBOSE(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit,
                            "%s:Service:%lusyncstatuschangedto:%s\n"),
                 FUNCTION_NAME(), n_channel_id, (n_sync==0?"LOPS":"AOPS")));
...
#if 0
	LOG_CLI((BSL_META_U(unit,
                            "%s:4 Service:%2d Encap:%d Label:%04lx Enable:%lu ret:0x%08x\n"), 
                 FUNCTION_NAME(),
                 ndMap.n_channel_id,
...
#if 0      
            LOG_CLI((BSL_META_U(unit,
                                "%s: rclock config flags=%x enbale=%d  "
...
#if 0
    LOG_CLI((BSL_META_U(unit,
                        "%s: rclock record config: enbale=%d   "
...
bcm_esw_ces_service_control_word_set(int unit, bcm_ces_service_t ces_service, uint16 tx_control_word_mask, uint16 tx_control_word) {
    ...
    LOG_VERBOSE(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit,
                            "%s: Service:%d mask:0x%04x LBit:0x%01x RBit:0x%01x "
                            "MBit:0x%01x\n"),
                 FUNCTION_NAME(),
                 msg.n_channel_id,
                 msg.n_bit_selector,
...
bcm_esw_ces_cb_register(int unit, 
    ...
    LOG_VERBOSE(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit,
                            "%s: Events:0x%02x cb:%p user_data:%p\n"),
                            FUNCTION_NAME(),
                 events,
                 (void *)callback,
                 user_data));
