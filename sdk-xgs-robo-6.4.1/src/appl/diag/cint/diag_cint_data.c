...
static int diag_modport2sysport(int unit, int func)
{
    ...
    if(SOC_IS_ARAD(unit)) {
        if (ARAD_IS_VOQ_MAPPING_INDIRECT(unit)) {
          cli_out("not supported in indirect mapping mode\n");
          return SOC_E_UNAVAIL;
            ...
            for (mod = 0; mod < ARAD_NOF_FAPS_IN_SYSTEM; ++mod) {
             ...
             if ( (rv = arad_sw_db_modport2sysport_destroy(unit)) ||
                 ...
                 cli_out("failed to clear the mapping. "
                         "It may now be unusable\n");
                 return SOC_E_FAIL;
             }
        } else if (func == 2 ) { /* populate mapping from hardware tables */
            if (arad_populate_sw_db_modport2sysport_from_hw_unsafe(unit)) {
                 cli_out("failed to populate the mapping.\n");
                 return SOC_E_FAIL;
            }
        } else {
            cli_out("argument must be one of:\n0 - print current mapping\n"
                    "1 - clear mapping\n2 - populate mapping from "
                    "hardware\n");
            return SOC_E_PARAM;
        }
    } else
...
static int diag_test_mc(int unit, char *test_name)
{
    ...
    cli_out("ERROR: unknown test name, use one of: "
            ...
            "print_all, print_errors_info, print_errors, assert, "
            "assert_0, assert_none, get_mc_asserts, get_nof_mcids, "
...
static int diag_test_mc2(int unit, char *test_name, int param)
{
    ...
    cli_out("ERROR: unknown test name, use one of: "
            "mc_replications get_irdb swdb_asserts_enable\n");
    return SOC_E_PARAM;
}
#endif /* BCM_88650_A0 */
