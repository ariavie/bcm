/*
 * $Id: d733418afc2d9bf12110eff01d53d3d4802abbc0 $
...
{
    if (file != NULL) {
        printk("<%s>", ((bslsink_sink_t *)file)->name);
    }
...
bslfile_init(void)
{
    ...
    sink->enable_range.max = bslSeverityWarn;

    /* Configure log prefix */
    sal_strncpy(sink->prefix_format, "%u:%L%S%s (%f:%F(%l)):",
