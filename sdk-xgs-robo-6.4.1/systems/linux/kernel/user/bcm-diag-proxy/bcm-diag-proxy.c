             ...
             unsigned int *len)
{
    ...
    static int cmd_counter=1;//debug
    if(_proxy_input_line[0]) {
	strcpy(data, _proxy_input_line);
	if (1 == cmd_counter) {//debug
       	    cmd_counter--; /* quit the program*/
            strcpy(_proxy_input_line, "quit\n");
        }else{  
...
main(int argc, char *argv[])
{
    ...
    if (argc == 1) {
        ...
        _fd_stdio_attach(sockfd);
            /*suporting bcm.user.proxy -i init_file*/   //dyk
    } else if (!strcmp(argv[1], "-i") && argc == 3) {
        strcpy(local_fname, argv[2]);
        FILE *fp = fopen(local_fname, "r");
        if (fp) {
                fclose(fp);
                strcpy(_proxy_input_line, "rcload ");
                strcat(_proxy_input_line, local_fname);
        } else {
                perror("fopen");
                strcpy(_proxy_input_line, "quit\n");
        }
