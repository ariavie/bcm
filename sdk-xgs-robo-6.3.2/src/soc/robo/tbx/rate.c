    ...
    uint32 burst_size)
{
    ...
    if (rate_type == TB_RATE_TYPE_PPS) {
        ...
        if ((limit > TB_RATE_METER_PKT_MAX(unit)) ||
            (limit < TB_RATE_METER_MIN(unit))) {
            soc_cm_debug(DK_ERR, 
                "_drv_tbx_port_erc_set : rate unsupported.\n");
            return  SOC_E_PARAM;
        }        
    ...
    if ((burst_size > TB_RATE_BURST_MAX(unit)) ||
        (burst_size < TB_RATE_BURST_MIN(unit))) {
        soc_cm_debug(DK_ERR, 
            "_drv_tbx_port_erc_set : burst size unsupported.\n");
        return SOC_E_PARAM;
    }
    ...
    if (limit == 0) {  /* Disable egress rate control for Total */
        ...
        if (rv < 0) {
            soc_cm_debug(DK_ERR, "_drv_tbx_port_erc_set : \
    ...
    uint32 kbits_sec_min, uint32 kbits_sec_max, uint32 burst_size)
{
    ...
    if (rate_type == TB_RATE_TYPE_PPS) {
        ...
        if ((kbits_sec_min > TB_RATE_METER_PKT_MAX(unit)) ||
            ...
            soc_cm_debug(DK_ERR, 
                "_drv_tbx_port_queue_erc_set : rate unsupported.\n");
            return  SOC_E_PARAM;
        }
        /*
          * COVERITY
    ...
    if ((burst_size > TB_RATE_BURST_MAX(unit)) ||
        (burst_size < TB_RATE_BURST_MIN(unit))) {
        soc_cm_debug(DK_ERR, 
            "_drv_tbx_port_queue_erc_set : burst size unsupported.\n");
        return SOC_E_PARAM;
    }
    ...
    if (kbits_sec_max == 0) {  /* Disable egress rate control for Q-n */
        ...
        if (field_val32) {
            ...
            if (rv < 0) {
                soc_cm_debug(DK_ERR, "_drv_tbx_port_queue_erc_set : \
    ...
    uint32 value)
{
    ...
    soc_cm_debug(DK_PORT, "drv_tbx_rate_config_set: \
    ...
    uint32 *value)
{
    ...
    soc_cm_debug(DK_PORT, "drv_rate_config_get: \
    ...
    uint32 kbits_sec_max, uint32 burst_size)
{
    ...
    soc_cm_debug(DK_PORT, "drv_tbx_rate_set: unit = %d, bmp = 0x%x, %s, flags = 0x%x, \
    ...
    uint32 *kbits_sec_max, uint32 *burst_size)
{
    ...
    soc_cm_debug(DK_PORT, "drv_tbx_rate_get: unit = %d, port = %d, %s, flags = 0x%x, \
        kbits_sec_min = %dK, kbits_sec_max = %dK, burst size = %dKB\n", 
