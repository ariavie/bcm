/*
* $Id: set_tdm.c 1.30.26.1 Broadcom SDK $
...
int write_mmu_tdm_tbl(int pgw0[32], int pgw1[32], int ovs0[32], int ovs1[32], int mmu[256], int bucket1[16], int bucket2[16], int bucket3[16], int bucket4[16], int x_pipe, int port_state_map[128], int tsc[NUM_WC][4], int bw, enum port_speed speed[NUM_EXT_PORTS], int (*op_flags)[1])
{
	...
	*/
	...
	if (y3 > 0)
	{
		...
		/* if ( (pgw_10==TRUE && s3%2==0) )
		{
			if ((s3)%(y3)==0)
			{
				y3_stretch=(s3)/(y3);
				if (6%(y3_stretch)!=0) overload10G=(6%(y3_stretch));
			}
			else overload10G=(s3-6);
			if (overload10G!=0) TDM_VERBOSE1("TDM: _____VERBOSE: overloaded 10G slots by +%d\n",overload10G);
			while (y3 > 0)
			{
				for (j=yy; j<yy+(4*s3); j+=s3) 
				{
					if (j<lr_idx_limit && mmu[j] == NUM_EXT_PORTS)
						mmu[j] = pgw40[y3];
					else
						TDM_WARN3("TDM: _____WARNING: 40G PORT #%0d at index %0d, already has %0d, buffering for rescheduling\n", pgw40[y3], j, mmu[j]);
				} */
				/* TDM_VERBOSE2("TDM: _____VERBOSE: y3_stretch set to %0d, yy pointer is at %0d\n",y3_stretch,yy); */
				/* yy+=y3_stretch; */
				/* for (j=yy; j<yy+48; j+=12) if (mmu[j] != NUM_EXT_PORTS) {yy++; continue;}
				if (pgw_100==TRUE) while (mmu[yy] != NUM_EXT_PORTS) yy++;
					if (pgw_100==TRUE) while (mmu[yy] != NUM_EXT_PORTS || yy % 2 == 0) yy++;
					while (mmu[yy] != NUM_EXT_PORTS) yy++;
				else while (mmu[yy] != NUM_EXT_PORTS || yy % 4 == 0) yy++; */
				/* y3--;
			}
		}
		...
		{ */
			while (y3 > 0) 
			{
				...
				/* proxcheck=FAIL;
				for (j=yy; j<yy+(4*s3); j+=s3) {x=0; if (mmu[j]==NUM_EXT_PORTS) x++; if (x==4) proxcheck==PASS;}
				if (proxcheck==PASS && (yy+s3)<lr_idx_limit) */
	...
	*/
	...
	if (y2 > 0)
	{
		...
		/* if ( (pgw_40==TRUE && s2%2==0) )
		{
			if ((s2)%(y2)==0)
			{
				y2_stretch=(s2)/(y2);
				if (6%(y2_stretch)!=0) overload10G=(6%(y2_stretch));
			}
			else overload10G=(s2-6);
			if (overload10G!=0) TDM_VERBOSE1("TDM: _____VERBOSE: overloaded 10G slots by +%d\n",overload10G);
			while (y2 > 0)
			{ */
				/* look ahead to line rate table length */
				/* proxcheck=FAIL;
				for (j=yy; j<yy+(2*s2); j+=s2) {x=0; if (mmu[j]==NUM_EXT_PORTS) x++; if (x==2) proxcheck==PASS;}
				if (proxcheck==PASS && (yy+s2)<lr_idx_limit)
					for (j=yy; j<yy+(2*s2); j+=s2) mmu[j] = pgw20[y2];
				else
				{
					TDM_WARN2("TDM: _____WARNING: 20G PORT #%0d at index %0d, failed to schedule, push into error buffer\n", pgw20[y2], j);
					ye++;
					err_buffer[ye]=pgw20[y2];
					err_speeds[ye]=20;
				} */
				/* TDM_VERBOSE2("TDM: _____VERBOSE: y2_stretch set to %0d, yy pointer is at %0d\n",y2_stretch,yy); */
				/* y+=y2_stretch; */
				/* for (j=yy; j<yy+48; j+=12) if (mmu[j] != NUM_EXT_PORTS) {yy++; continue;}
				if (pgw_100==TRUE) while (mmu[yy] != NUM_EXT_PORTS) yy++;
					if (pgw_100==TRUE) while (mmu[yy] != NUM_EXT_PORTS || yy % 2 == 0) yy++;
		...
		{ */
			while (y2 > 0)
			{
				...
				/* Rollback table pointer for different spacing rule */
				/* for (v=0; v<lr_idx_limit; v++) {x=0; for (w=v; w<v+(2*s2); w+=s2) if (mmu[w] == NUM_EXT_PORTS) x++; if (x==2) {yy=v; break;}} */
				/* look ahead to line rate table length */
				/* proxcheck=fail; x=0;
				for (j=yy; j<yy+(2*s2); j+=s2) {if (mmu[j]==num_ext_ports) x++; if (x==2) {proxcheck==pass; break;}}
				if (proxcheck==pass && (yy+s2)<lr_idx_limit)
				if ((yy+s2)<lr_idx_limit)
				{
					for (j=yy; j<yy+(2*s2); j+=s2) mmu[j] = pgw20[y2];
					printf("20G port #%0d put at index %0d\n",pgw20[y2],yy);
				}
				else
				{
					TDM_WARN2("TDM: _____WARNING: 20G PORT #%0d at index %0d, failed to schedule, push into error buffer\n", pgw20[y2], j);
				...
				while (mmu[yy]!=NUM_EXT_PORTS||mmu[yy+s2]!=NUM_EXT_PORTS) yy++; */
				/* for (j=yy; j<yy+48; j+=24) if (mmu[j] != NUM_EXT_PORTS) {yy++; continue;}
				if (pgw_100==TRUE) while (mmu[yy] != NUM_EXT_PORTS) yy++;
				if (pgw_100==TRUE) while (mmu[yy] != NUM_EXT_PORTS || yy % 2 == 0) yy++;
				while (mmu[yy] != NUM_EXT_PORTS) yy++;
				else while (mmu[yy] != NUM_EXT_PORTS || yy % 4 == 0) yy++; */
	...
	/* alternatie algorithm for scheduling 10G based on swap search */
	/*
		10G
	*/
	/* if (y1 > 0)
	{
		...
		/* if ( (bw==960 && overload10G==0 && pgw_20==FALSE) )
		{
			TDM_VERBOSE0("TDM: _____VERBOSE: overloaded 10G slots by +6\n");
			overload10G=6;
		}
		...
		else if ( (pgw_40==TRUE && s3%6!=0) )
		{
			overload10G=(s3-6);
			TDM_VERBOSE1("TDM: _____VERBOSE: overloaded 10G slots by +%0d\n",overload10G);
		}
		else if ( (pgw_20==TRUE && s2%6!=0) )
		{
			overload10G=(s2-6);
			TDM_VERBOSE1("TDM: _____VERBOSE: overloaded 10G slots by +%0d\n",overload10G);
		} */
		/* else if ( (bw==720 && y1%4==0) ) 
		{
			TDM_VERBOSE0("TDM: _____VERBOSE: overloaded 10G slots by +3\n");
			overload10G=3;
		} */
		/* else if (  ) 
		{
			TDM_WARN0("TDM: _____WARNING: applying 10G overload +4\n");
			overload10GP4=4;
		} */
		/* while (y1 > 0)
		{
			...
			{ */
				...
				/* retrieve all ports from bucket that originate from the same TSC */
				/* for (b=0; b<4; b++) if (tsc[c][b]!=temp_10G[0] && tsc[c][b]!=NUM_EXT_PORTS) for (a=1; a<(y1+2); a++) if (pgw10[a]==tsc[c][b])
				{
					d=a;
					e++;
					temp_10G[e] = tsc[c][b];
			...
			if (e<3) for (v=0; v<=1; v++) if (tsc[c][v]!=NUM_EXT_PORTS) if (tsc[c][v]==tsc[c][v+2]) {triports=TRUE; TDM_VERBOSE0("TDM: _____partial x10G warp core detected\n");}	
			...
			while (proxcheck!=PASS)
			{
				...
				for (v=yy; v<(yy+(6+overload10G)*(e+1)); v+=(6+overload10G))
				{
					...
					if (triports==TRUE)
					{
						for (a=1; a<6; a++) for (jjj=0; jjj<4; jjj++) 
						{
							if (mmu[v+a]==tsc[c][jjj]) proxcheck=FAIL;
							if (v>=a) if (mmu[v-a]==tsc[c][jjj]) proxcheck=FAIL;
						}
					}
				}
	...
	/* error buffer search for best fit, prioritize empty slots, then swap filled slots */
	/* if (ye>0)
	{
		TDM_VERBOSE0("TDM: __________VERBOSE: error buffer not empty, will attempt to search best fit solution\n");
		for (j=1; j<=ye; j++) printf("error buffer index %0d has port %0d which is %0dG\n",j,err_buffer[j],err_speeds[j]);
		limiter=65535;
		while (ye>0)
		{
			l=ye;
			switch (err_speeds[ye])
			{
				case 10: */
					/* all 10G from the same TSC will be consecutive in the array */
					/* break;
				case 20: */
					/* search free slots */
					/* for (i=0; i<lr_idx_limit; i++) if (mmu[i]==NUM_EXT_PORTS && (i+s2)<lr_idx_limit)
					{
						for (j=i; j<i+(2*s2); j+=s2) {x=0; if (mmu[j]==NUM_EXT_PORTS) x++;} 
						if (x==2) 
						{
							for (a=i; a<i+(2*s2); a+=s2) mmu[a]=err_buffer[ye];
							ye--; 
							break;
						}
					} */
					/* if we were unsucessful, go into swap search algorithm */
					/* if (l==ye)
					{ */
						/* search for swappable slots subject to the following conditions:
						1. only swap single ports (otherwise cannot achieve parity)
						2. do not swap ports of the same speed 
						3. all slots can be either empty or contain the same port */
						/* for (i=0; i<lr_idx_limit; i++) if ( (i+s2)<lr_idx_limit && speed[mmu[i]]!=SPEED_20G && ( mmu[i]!=NUM_EXT_PORTS || mmu[i+s2]!=NUM_EXT_PORTS) )
						{
							if (mmu[i+s2]!=mmu[i]) continue;
							swapcheck=PASS;
							for (j=0; j<64; j++) if (err_swap[j]==i) swapcheck=FAIL; */
							/* if (swapcheck==FAIL && ys>8) ys=0; */
							/* else if (swapcheck==FAIL && ys<=8) continue; */
							/* if (swapcheck==FAIL) continue;
							else
							{
								for (j=0; j<2; j++) if (mmu[i+j*s2]!=NUM_EXT_PORTS) g = mmu[i+j*s2];
								switch (speed[g])
								{
									case SPEED_40G: */
										/* search to extract ensconced port */
										/* for (j=0; j<lr_idx_limit; j++) if (mmu[j]==g) mmu[j]==NUM_EXT_PORTS;
										for (j=i; j<i+(2*s2); j+=s2) mmu[j]=err_buffer[ye]; */
										/* save swap index to prevent cyclic redundancy */
										/* ys++;
										err_swap[ys]=i;
										printf("swapping 20G port %0d in buffer index %0d with 40G port %0d at index %0d\n",err_buffer[ye],ye,mmu[i],i); */
										/* swap ports */
										/* err_buffer[ye]=g;
										break;
									default:
					...
					break;
				case 40:
					for (i=0; i<lr_idx_limit; i++) if (mmu[i]==NUM_EXT_PORTS && (i+(3*s3))<lr_idx_limit)
					{
						for (j=i; j<i+(4*s3); j+=s3) {x=0; if (mmu[j]==NUM_EXT_PORTS) x++;} 
						if (x==4) 
						{
							for (a=i; a<i+(4*s3); a+=s3) mmu[a]=err_buffer[ye];
							ye--; 
							break;
						}
					}
					if (l==ye)
					{
						for (i=0; i<lr_idx_limit; i++) if ( (i+(3*s3))<lr_idx_limit && speed[mmu[i]]!=SPEED_40G && ( mmu[i]!=NUM_EXT_PORTS || mmu[i+s3]!=NUM_EXT_PORTS || mmu[i+(2*s3)]!=NUM_EXT_PORTS || mmu[i+(3*s3)]!=NUM_EXT_PORTS) )
						{
							proxcheck=FAIL;
							for (j=0; j<4; j++) if (mmu[i+(j*s3)]!=NUM_EXT_PORTS) {proxid=mmu[i+(j*s3)]; break;}
							for (j=0; j<4; j++) if (mmu[i+(j*s3)]==NUM_EXT_PORTS || mmu[i+(j*s3)]==proxid) proxcheck=PASS;
							if (proxcheck==FAIL) continue;
							swapcheck=PASS;
							for (j=0; j<64; j++) if (err_swap[j]==i) swapcheck=FAIL; */
							/* if (swapcheck==FAIL && ys>8) ys=0;
							...
							{
								for (j=0; j<4; j++) if (mmu[i+j*s3]!=NUM_EXT_PORTS) g = mmu[i+j*s3];
								switch (speed[g])
								{
									case SPEED_20G:
										for (j=0; j<lr_idx_limit; j++) if (mmu[j]==g) mmu[j]==NUM_EXT_PORTS;
										for (j=i; j<i+(4*s3); j+=s3) mmu[j]=err_buffer[ye];
										ys++;
										err_swap[ys]=i;
										printf("swapping 40G port %0d in buffer index %0d with 20G port %0d at index %0d\n",err_buffer[ye],ye,mmu[i],i);
										err_buffer[ye]=g;
										break;
									default:
										continue;
		...
		if (ye>0)
		{
			TDM_ERROR1("TDM: _____ERROR: search was unable to find best fit solution, ye pointer left at %0d\n",ye);
			for (j=1; j<=ye; j++) printf("new error buffer index %0d has port %0d which is %0dG\n",j,err_buffer[j],err_speeds[j]);
			return 0;
		}
	} */
	...
	*/
	if (y1 > 0)
	{
		...
		else
		{
			...
			while (y1 > 0)
			{
				...
				/* if (e<3) for (v=0; v<=1; v++) if (tsc[c][v]!=NUM_EXT_PORTS) if (tsc[c][v]==tsc[c][v+2]) {triports=TRUE; TDM_VERBOSE0("TDM: _____partial x10G warp core detected\n");}
				for (jjj=0; jjj<4; jjj++) printf("tsc[%0d][%0d] has %0d\n",c,jjj,tsc[c][jjj]); */
				b=e+1;
				limiter=65535;
				while (b>0)
				{
				...
				for (a=0; a<(e+1); a++) 
				{
					if (mmu[yy] == NUM_EXT_PORTS) {; yy+=(6+overload10G);}
					else
						TDM_WARN3("TDM: _____WARNING: 10G PORT #%0d at index %0d, already has %0d, buffering for rescheduling\n", temp_10G[a], yy, mmu[yy]);
				}
				else
	...
	for (j=255; j>0; j--)
	{
		...
		/* for (k=0; k<24; k++) if (mmu[j+k] != NUM_EXT_PORTS) i++;
		if (i==0)
		{
			i = j;
			break;
	...
	else if (speed[mmu[i-1]]>=SPEED_20G)
	{
		...
		if (pgw_100==TRUE)
		{
			...
			/* if (pgw_100==TRUE && m==5) i+=(4-e);
			if (pgw_100==TRUE && m==4) i+=(6-e);
			else if (pgw_100==TRUE && m==6) i+=(4-e); */
		}
		else if (e > m) {}
		else i+=(m-e);
	}
...
/* 		s1=(int)((49.5*(clk/755.0))+0.5); */
	...
	*/
	...
	for (j=0; j<(4*i); j++)
	{
		if (mmu[j] == MGM_TOKEN)
		{
			if (tsc[3][1] == MGM_TOKEN)
			{
				if (j<i) mmu[j] = 13;
				else if (j>=(1*i) && j<(2*i)) mmu[j] = 14;
				else if (j>=(2*i) && j<(3*i)) mmu[j] = 15;
				else if (j>=(3*i) && j<(4*i)) mmu[j] = 16;
			}
			else mmu[j] = 13;
		}
	}
	if (tsc[3][0] == MGM_TOKEN && x_pipe) mmu[4*i] = 0;
	*/
	/* 	int ep_ovs_cnt = 0; */
	/* 	for (j=0; j<(4*i); j++) if (mmu[j]==OVS_TOKEN) ep_ovs_cnt++; */
	/*	for (j=0; j<(4*i); j++) if (mmu[j]==IDL_TOKEN) ep_idl_cnt++; */
	/* if (ep_ovs_cnt > 5 && ep_idl_cnt < 5)
	{
		ep_idl_put = 5-ep_idl_cnt;
		while (ep_idl_put > 0)
		{
			for (j=0; j<(4*i); j++)
			{
	...
	else if (ep_ovs_cnt <= 5 && ep_idl_cnt < 5) */
	...
	if (ep_idl_cnt < 3)
	{
	ep_idl_put = 3-ep_idl_cnt;
	TDM_WARN1("TDM: _____WARNING: post-schedule insertion of %0d idle tokens, LOCAL SPACING MAY BE VIOLATED\n", ep_idl_put);
		for (j=1; j<ep_idl_put+1; j++)
		{
			ep_idl_idx = j*(TDM_scheduler_PQ(((4*i+ep_idl_put-1)*10)/(ep_idl_put+1)));
			ep_idl_idx = j*(TDM_scheduler_PQ((float) ((4.0*((float)i))+((float)ep_idl_put)-1.0)/(((float)ep_idl_put)+1.0)));
					for (k=1; k<yT+1; k++) if (mmu[ep_idl_idx+yT]== 13 ||
								mmu[ep_idl_idx+yT]== 14 ||
								mmu[ep_idl_idx+yT]== 15 ||
								mmu[ep_idl_idx+yT]== 16 ||
								mmu[ep_idl_idx-yT]== 13 ||
								mmu[ep_idl_idx-yT]== 14 ||
								mmu[ep_idl_idx-yT]== 15 ||
								mmu[ep_idl_idx-yT]== 16) 	
			TDM_VERBOSE1("TDM: __________VERBOSE: inserting ep idle token at index %0d\n", ep_idl_idx);
			for (k=254; k>=ep_idl_idx; k--) mmu[k+1] = mmu[k];
			mmu[ep_idl_idx] = IDL_TOKEN;
		}
		mmu[(4*i)+ep_idl_put] = (x_pipe) ? 0 : 129;
	}
	*/
	...
	while (z6 > 0)
	{
		...
		while (z6<16 && z6>0)
		{
			...
			z6-=(1+i);
			/* dequeue into buckets subject to given constraints */
			/* code now redundant with above loop */
			if (z11 < 4)
			{
				for (m = 0; m < 4; m++) /* if (tsc_swap[m] != NUM_EXT_PORTS) */ bucket1[z11+(m*4)] = tsc_swap[m];
				z11++;
		...
		while (z1>=16)
		{
			for (v=0; v<4; v++)
			{
				...
				for (m = 0; m < 4; m++)
				{
					for (j = 0; j < z1; j++)
					{
						if (tsc[((tsc_swap[0])/4)-1][m] != tsc_swap[0] && tsc[((tsc_swap[0])/4)-1][m] == ovs10[j])
						{
							i++;
							tsc_swap[i] = ovs10[j];
							for (h = j; h < z1; h++) ovs10[h] = ovs10[h+1];
						}
					}
				}
				*/
	...
	while (z2 > 0)
	{
		...
		if (speed[tsc[c][0]]<SPEED_20G || speed[tsc[c][2]]<SPEED_20G || speed[tsc[c][1]]<SPEED_20G || speed[tsc[c][3]]<SPEED_20G) {
			...
			while (z2>=8)
			{
				for (v=0; v<4; v++)
				{
					...
					for (m = 0; m < 4; m++)
					{
						for (j = 0; j < z2; j++)
						{
							if (tsc[((tsc_swap[0])/4)-1][m] != tsc_swap[0] && tsc[((tsc_swap[0])/4)-1][m] == ovs20[j])
							{
								i++;
								tsc_swap[i] = ovs20[j];
								for (h = j; h < z2; h++) ovs20[h] = ovs20[h+1];
							}
						}
					}
					*/
			...
			while (z2<8 && z2>0)
			{
				...
				printf("tsc_swap[1] is %0d\n",tsc_swap[1]);
				printf("i is %0d z2 is %0d\n",i,z2);
				*/
				z2-=(1+i);
	...
	/* shuffle ovs buckets contents to ensure TSC discordance */
	/*
	zSS=0; 
	if (z11!=0)
	{
		while (z_shuffle_done==FALSE)
		{
			z_shuffle_gap=0;
			for (i=0; i<16; i++) {z_shuffle_swap[i]=NUM_EXT_PORTS; z_shuffle_map[i]=NUM_EXT_PORTS; z_shuffle_idx[i/4]=NUM_EXT_PORTS;}
			for (i=0; i<z11; i++) if (bucket1[i]!=NUM_EXT_PORTS) 
			{
				z_shuffle_gap++;
				if (bucket1[i]!=IDL_TOKEN) for (h=0; h<NUM_WC; h++)
				{
					for (m=0; m<4; m++)
					{
						if (tsc[h][m]==bucket1[i])
						{
								z_shuffle_map[i]=h;
								break;
						}
					}
					if (z_shuffle_map[i]!=NUM_EXT_PORTS) break;
				}
			}
			if (z_shuffle_gap%4!=0) z_shuffle_gap=(z_shuffle_gap+(4-(z_shuffle_gap%4)));
			z_shuffle_gap/=4;
			m=0;
			z_shuffle_done=TRUE;
			for (i=0; i<16; i++) if (z_shuffle_map[i]!=NUM_EXT_PORTS && (z_shuffle_map[i]==z_shuffle_map[i-1]||z_shuffle_map[i]==z_shuffle_map[i+1]))
			{
				z_shuffle_done=FALSE;
				z_shuffle_idx[m]=i;
				m++;
				if (m>0 && z_shuffle_map[i]!=z_shuffle_idx[0]) break;
			}
			if (m>1) for (h=0; h<4; h++) if (z_shuffle_idx[h]!=NUM_EXT_PORTS)
			{
				z_shuffle_swap[zSS]=bucket1[z_shuffle_idx[h]];
				zSS+=z_shuffle_gap;
				bucket1[z_shuffle_idx[h]]=IDL_TOKEN;
			}
			for (i=0; i<16; i++) if (z_shuffle_swap[i]==NUM_EXT_PORTS) {zSS=i; break;}
			for (i=0; i<z11; i++) if (bucket1[i]!=NUM_EXT_PORTS) w++;
			for (i=0; i<16; i++) if (z_shuffle_swap[i]!=NUM_EXT_PORTS) v++;
			if (w==v) z_shuffle_done=TRUE;
		}
		for (i=0; i<16; i++)
		{
			if (bucket1[i]!=IDL_TOKEN && bucket1[i]!=NUM_EXT_PORTS)
			{
				for (h=0; h<16; h++) if (z_shuffle_swap[h]==NUM_EXT_PORTS) {z_shuffle_swap[h]=bucket1[i]; break;}
			}
		}
	} 
	*/
...
/* ----------------------------------API------------------------------------- */
	...
	**/
	int set_tdm_tbl(enum port_speed speed[NUM_EXT_PORTS], int tdm_bw, 
			...
			int iarb_tdm_tbl_x[512], int iarb_tdm_tbl_y[512])
	{
		int wc_array[NUM_WC][4] = {{ 0 }}, mgmtbw=0;
		int i;
		int checkpoint[6];
		int op_flags_x[1];
		int op_flags_y[1];
