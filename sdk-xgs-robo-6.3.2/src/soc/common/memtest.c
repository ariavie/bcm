/*
 * $Id: memtest.c 1.45.2.2 Broadcom SDK $
...
ser_test_mem_read(int unit, ser_test_data_t *test_data)
{
    ...
    if (test_data->acc_type != _SOC_ACC_TYPE_PIPE_ANY) {
        ...
        rv = soc_mem_read(unit, test_data->mem, test_data->mem_block,
                test_data->index, test_data->entry_buf);
    }
    if (SOC_FAILURE(rv)) {
        soc_cm_debug(DK_ERR,
...
ser_test_mem_pipe(int unit, soc_reg_t parity_enable_reg, int tcam_parity_bit,
                            ...
                            soc_port_t port, acc_type_t acc_type, int *error_count)
{
...
soc_ser_create_test_data(int unit, uint32 *tmp_entry, uint32 *fieldData,
                              ...
                              soc_port_t port, acc_type_t acc_type,
                              ser_test_data_t *test_data)
{
    test_data->mem = mem;
...
#ifdef SOC_MEM_NAME
    test_data->mem_name   = SOC_MEM_NAME(unit, test_data->mem);
    test_data->field_name = SOC_FIELD_NAME(unit, test_data->test_field);
#else
...
{
    int numIndexesToTest, j, read_through, startErrorCount, mem_err;
    int indexesToTest[3];
    ...
    for (j = 0; j < numIndexesToTest; j++) {
        ...
        /*Enable NACK on read */
        mem_err = SOC_MEM_RETURN_SCHAN_ERR(unit, test_data->mem);
        SOC_MEM_RETURN_SCHAN_ERR_SET(unit, test_data->mem, TRUE);
        (void)ser_test_mem_read(unit, test_data);
        SOC_MEM_RETURN_SCHAN_ERR_SET(unit, test_data->mem, mem_err);
 ...
 */
soc_error_t
soc_ser_inject_error(int unit, soc_mem_t mem, acc_type_t acc_type,
                     int blk, int index)
{
    if( ser_test_functions[unit] != NULL &&
