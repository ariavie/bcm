/*
 * $Id: phy84328.c 1.121 Broadcom SDK $
...
static unsigned int phy84328_maj_rev = 1121;
...
STATIC int
_phy_84328_polarity_flip_tx_reg_verify(int unit, soc_port_t port, int flip)
{
	uint16		tmp, data, mask;
	phy_ctrl_t *pc = EXT_PHY_SW_STATE(unit, port);

	/* Clear to flip, set to unflip */
	if (CUR_DATAPATH(pc) == PHY84328_DATAPATH_20) {
		SOC_IF_ERROR_RETURN(
			READ_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_ANATXACONTROL, &data));
		tmp  = flip ? 0 : PHY84328_DEV1_ANATXACONTROL_TXPOL_FLIP_MASK;
		mask = PHY84328_DEV1_ANATXACONTROL_TXPOL_FLIP_MASK;
	} else {
		SOC_IF_ERROR_RETURN(
			READ_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_ANATXACONTROL4, &data));
		tmp  = flip ? 0 : PHY84328_DEV1_ANATXACONTROL4_XFI_LOWLATENCY_POLARITY_FLIP_MASK;
		mask = PHY84328_DEV1_ANATXACONTROL4_XFI_LOWLATENCY_POLARITY_FLIP_MASK;
	}
    if (tmp != (data & mask)) {
...
STATIC int
_phy_84328_polarity_flip_rx_20bit_verify(int unit, soc_port_t port, int flip)
{
	uint16		tmp, data, mask;
	phy_ctrl_t *pc = EXT_PHY_SW_STATE(unit, port);
    SOC_IF_ERROR_RETURN(
	READ_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_GP_REGISTER_RX_INV, &data));
	/* Default is clear, set to flip it */                                  \
	tmp  = flip ? (PHY84328_DEV1_GP_REGISTER_RX_INV_FORCE | PHY84328_DEV1_GP_REGISTER_RX_INV_INVERT) : 0;                                                   \
	mask = (PHY84328_DEV1_GP_REGISTER_RX_INV_FORCE | PHY84328_DEV1_GP_REGISTER_RX_INV_INVERT);
    if (tmp != (data & mask)) {
        return SOC_E_INTERNAL;
    }
...
STATIC int
_phy_84328_polarity_flip_rx_4bit_verify(int unit, soc_port_t port, int flip)
{
	uint16		tmp, data, mask;
	phy_ctrl_t *pc = EXT_PHY_SW_STATE(unit, port);
    SOC_IF_ERROR_RETURN(
...
STATIC int
_phy_84328_polarity_flip_verify(int unit, soc_port_t port, uint16 cfg_tx_pol, uint16 cfg_rx_pol)
{
	int			flip;
	phy_ctrl_t *pc = EXT_PHY_SW_STATE(unit, port);
    ...
    if ((PHY84328_SINGLE_PORT_MODE(pc))) {
		...
		/* 4-bit polarity handled on system side */
		if (CUR_DATAPATH(pc) != PHY84328_DATAPATH_20) {
			PHY_84328_MICRO_PAUSE(unit, port, "polarity verify");
		}
        for (lane = 0; lane < PHY84328_NUM_LANES; lane++) {
			flip = 0;
			if ((cfg_rx_pol == POL_CONFIG_ALL_LANES) || 
				((cfg_rx_pol & POL_CONFIG_LANE_MASK(lane)) == POL_CONFIG_LANE_MASK(lane))) {
				flip = 1;
			}
			if (CUR_DATAPATH(pc) == PHY84328_DATAPATH_20) {
				SOC_IF_ERROR_RETURN(_phy_84328_channel_select(unit, port, PHY84328_INTF_SIDE_LINE, lane));
				SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_rx_20bit_verify(unit, port, flip));
			} else {
				SOC_IF_ERROR_RETURN(_phy_84328_channel_select(unit, port, PHY84328_INTF_SIDE_SYS, lane));
				SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_rx_4bit_verify(unit, port, flip));
			}
			flip = 0;
			if ((cfg_tx_pol == POL_CONFIG_ALL_LANES) || 
				((cfg_tx_pol & POL_CONFIG_LANE_MASK(lane)) == POL_CONFIG_LANE_MASK(lane))) {
				flip = 1;
			}
			/* Select the lane on the line side */
			SOC_IF_ERROR_RETURN(_phy_84328_channel_select(unit, port, PHY84328_INTF_SIDE_LINE, lane));
			SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_tx_reg_verify(unit, port, flip));
			SOC_DEBUG_PRINT((DK_PHY, "84328 polarity flip verify: u=%d p=%d lane=%d\n", unit, port, lane));
		}
		/* Restore to default single port register access */
		SOC_IF_ERROR_RETURN(_phy_84328_channel_select(unit, port, PHY84328_INTF_SIDE_LINE, PHY84328_ALL_LANES));
		_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
		...
		flip = (cfg_rx_pol == POL_CONFIG_ALL_LANES);
		if (CUR_DATAPATH(pc) == PHY84328_DATAPATH_20) {
			_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
			SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_rx_20bit_verify(unit, port, flip));
		} else {
		...
		flip = (cfg_tx_pol == POL_CONFIG_ALL_LANES) ? 1 : 0;
		SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_tx_reg_verify(unit, port, flip));
		SOC_DEBUG_PRINT((DK_PHY, "84328 rx polarity flip verify: u=%d p=%d\n", unit, port));
	}
...
_phy_84328_init_pass2(int unit, soc_port_t port)
{
    ...
    _phy_84328_micro_pause(unit, port, "polarity config");
    ...
    /* Verify Polarity Flips are still programmed correctly  */
    SOC_IF_ERROR_RETURN(
	_phy_84328_polarity_flip_verify(unit, port, POL_TX_CFG(pc), POL_RX_CFG(pc)));
...
_phy_84328_rx_los_control_set(int unit, soc_port_t port, uint32 value)
{
    ...
    if (DEVREV(pc) == 0x00a0) {
        rv = _phy_84328_sw_rx_los_control_set(unit, port, value);
    } else {
        rv = _phy_84328_fw_rx_los_control_set(unit, port, value);
    }
