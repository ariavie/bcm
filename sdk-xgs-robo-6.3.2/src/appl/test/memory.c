/*
 * $Id: memory.c 1.91.2.1 Broadcom SDK $
...
#define _ERR_MSG_MODULE_NAME SOC_DBG_MEM
...
memories_rw_test(int unit, args_t *a, void *pa)
{
        ...
        soc_cm_debug(DK_TESTS, "Memories read/write test\n");
        ...
        if (SOC_IS_ARAD(unit))
        {
          ...
          if ((rv = soc_dpp_device_reset(unit, SOC_DPP_RESET_MODE_REG_ACCESS,SOC_DPP_RESET_ACTION_INOUT_RESET)) < 0) {
              soc_cm_debug(DK_ERR, "ERROR: Unable to reinit unit %d \n", unit);
...
#define DISABLE_ECC_CHECK_FUNC_RESULT(res, err_num,op, err_val) \
    ...
    if (res != SOC_E_NONE) { \
        ...
        soc_cm_debug(DK_ERR, "error in function disable_ecc_dynamic_mechanism()\n"); \
        goto exit;  \
    }
...
{\
  uint32 reg_value; \
  DISABLE_ECC_CHECK_FUNC_RESULT(res,err_num, soc_reg32_get(device_id, reg_desc, (REG_PORT_ANY), 0, &reg_value) ,\
                ARAD_REG_ACCESS_ERR); \
  soc_reg_field_set(device_id, reg_desc, &reg_value,     fld_desc, fld_value); \
  DISABLE_ECC_CHECK_FUNC_RESULT(res, err_num, soc_reg32_set(device_id, reg_desc, REG_PORT_ANY, 0, reg_value),\
                                             ARAD_REG_ACCESS_ERR); \
}



#define DISABLE_ECC_ARAD_REG_GET(reg_desc, index, reg_value, err_num)  \
{                                                                 \
  reg_value = 0; \
  DISABLE_ECC_CHECK_FUNC_RESULT(res, err_num,soc_reg32_get(device_id, reg_desc, REG_PORT_ANY, index, &(reg_value)), \
     ARAD_REG_ACCESS_ERR);\
}
...
{\
  uint32 reg_value; \
  DISABLE_ECC_ARAD_REG_GET(reg_desc, index, reg_value, err_num); \
  soc_reg_field_set(device_id, reg_desc, &reg_value, fld_desc, fld_value); \
  DISABLE_ECC_ARAD_FIELD_SET(reg_desc, index,reg_value, err_num); \
}
...
disable_ecc_disable_dynamic_mechanism(int device_id)
{
  ...
  DISABLE_ECC_SOC_SAND_IF_ERROR_RETURN(res, 20, WRITE_EGQ_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  DISABLE_ECC_SOC_SAND_IF_ERROR_RETURN(res, 30,  WRITE_EPNI_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  DISABLE_ECC_SOC_SAND_IF_ERROR_RETURN(res, 40,  WRITE_IDR_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  DISABLE_ECC_SOC_SAND_IF_ERROR_RETURN(res, 50,  WRITE_IHB_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  DISABLE_ECC_SOC_SAND_IF_ERROR_RETURN(res, 60, WRITE_IPS_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  DISABLE_ECC_SOC_SAND_IF_ERROR_RETURN(res, 70,  WRITE_IPT_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  DISABLE_ECC_SOC_SAND_IF_ERROR_RETURN(res, 80,  WRITE_IQM_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
...
memories_rw_first_last_test(int unit, args_t *a, void *pa)
{
   ...
   COMPILER_REFERENCE(pa);
        ...
        soc_cm_debug(DK_TESTS, "Memories read/write first and last test\n");
        ...
        if (SOC_IS_ARAD(unit))
        {
...
#endif
          {
              ...
              soc_cm_debug(DK_TESTS, "Skip device reset\n");
          }
...
mem_flipflop_test(int unit, args_t *a, void *pa)
{
   ...
   help_status = 0;
        ...
        /* COMPILER_REFERENCE(pa); */
        soc_cm_debug(DK_TESTS, "Memories read/write first and last test, %s\n",ctime((const time_t *)&current_time));
...
int ser_test_init(int unit, args_t *arg, void **p) {
    ...
    if (!sal_strcasecmp(test_type_name, "single")) {
        ...
        soc_cm_print("Invalid test type selected.\n");
        return -1;
    }
...
int ser_test(int unit, args_t *arg, void *p) {
    ...
    uint32 my_debug_level = debugk_enable(0);
    int rv = SOC_E_NONE;
    int force_read_through = SOC_MEM_FORCE_READ_THROUGH(unit);
    debugk_disable(DK_ERR);
    debugk_disable(DK_VERBOSE);
    ...
    if (test_params->mem != INVALIDm) {
        if (test_params->inject_only == TRUE) {
            soc_ser_inject_error(unit, test_params->mem, 0,-1,0);
        } else {
        ...
        if (test_params->inject_only == TRUE) {
            soc_cm_print(
                "Parameter 'injectOnly' may only be used when 'memory' is also specified.\n");
        } else {
            rv = soc_ser_test(unit, test_params->test_type);
        }
    }
    /* Restore previous debug settings */
    debugk_select(my_debug_level);
    if (!force_read_through) {
