...
/*
 * Description:
 *     Add a nexthop entry for a bcm_fcoe_route_t structure
 * Parameters:
 ...
 */
STATIC int
_bcm_fcoe_add_nh_entry(int unit, bcm_fcoe_route_t *route, int *index)
{
    _bcm_l3_cfg_t l3cfg;
    int rv = BCM_E_NONE;
    ...
    /* 
     * Sometimes _bcm_xgs3_nh_init_add will return BCM_E_NONE even when the
     * parameters are invalid.  Test for that case here... 
     */
    if (*index == BCM_XGS3_L3_INVALID_INDEX) {
        return BCM_E_PARAM;
    }
 ...
 */
STATIC int
_bcm_fcoe_route_construct_key_prefix(int unit, soc_mem_t mem, uint32 *table_row,
                                     bcm_fcoe_route_t *route)
{
    if (route->vrf >= SOC_VRF_MAX(unit) || route->vrf == 0)
        return BCM_E_PARAM;
    ...
    soc_mem_field32_set(unit, mem, table_row, FCOE__D_IDf,
                        route->nport_id & route->nport_id_mask);
    soc_mem_field32_set(unit, mem, table_row, ENTRY_TYPEf, 1); /* 1 = FCOE */
    soc_mem_field32_set(unit, mem, table_row, VALIDf, 1);
...
/*
 * Description:
 *     Construct an L3 table key (FCOE or FCOE_EXT view) from the given route
 * Parameters:
 *     unit      - (IN) device number
 *     mem       - (IN) memory (table) used
 *     table_row - (OUT) buffer holding a table row
 ...
 */
int
_bcm_fcoe_route_construct_key(int unit, soc_mem_t mem, uint32 *table_row,
                                  bcm_fcoe_route_t *route)
{
    if (_bcm_td2_fcoe_route_is_prefix(unit, route)) {
        return _bcm_fcoe_route_construct_key_prefix(unit, mem, table_row, 
                                                    route);
    } else if (BCM_XGS3_L3_EGRESS_MODE_ISSET(unit) == 0) {
        return _bcm_fcoe_route_construct_key_ext(unit, mem, table_row, route);
    } else {
        return _bcm_fcoe_route_construct_key_normal(unit, mem, table_row,
                                                    route);
    }
}
...
_bcm_td2_fcoe_route_add_normal(int unit, bcm_fcoe_route_t *route)
{
    ...
    /* add nexthop entry and retrieve the new index */
    BCM_IF_ERROR_RETURN(_bcm_fcoe_add_nh_entry(unit, route,
                                               &next_hop_index));

    if (route->flags & BCM_FCOE_MULTIPATH) {
        soc_mem_field32_set(unit, mem, buf, FCOE__NEXT_HOP_INDEXf,
                            next_hop_index);
        soc_mem_field32_set(unit, mem, buf, FCOE__ECMPf,
                            (route->flags & BCM_FCOE_MULTIPATH) ? 1 : 0);
    } else {
        soc_mem_field32_set(unit, mem, buf, FCOE__NEXT_HOP_INDEXf,
                            next_hop_index);
    }
...
bcm_td2_fcoe_route_add(int unit, bcm_fcoe_route_t *route)
{
    ...
    if (_bcm_td2_fcoe_route_is_prefix(unit, route)) {
        ...
        return _bcm_td2_fcoe_route_add_ext(unit, route);
    } else {
        return _bcm_td2_fcoe_route_add_normal(unit, route);
    }
#else
    return BCM_E_UNAVAIL;
#endif
...
{
#ifdef INCLUDE_L3
    return _bcm_td2_fcoe_traverse_table(unit, L3_ENTRY_IPV4_MULTICASTm,
...
bcm_td2_fcoe_vsan_create (int unit,  bcm_fcoe_vsan_t *vsan)
{
    ...
    if (vsan->flags & BCM_FCOE_VSAN_NORMALIZED_CHECK) {
                       ...
                       bcm_fcoe_vsan_t *vsan)
{
    ...
    flag = soc_mem_field32_get(unit, mem, (uint32 *)&vsan_entry, 
                               ZONE_CHECK_NORMALIZEDf);
    if (flag) {
        vsan->flags |= BCM_FCOE_VSAN_NORMALIZED_CHECK;
    }
    flag = soc_mem_field32_get(unit, mem, (uint32 *)&vsan_entry, 
                               FCOE_ROUTE_ENABLEf);
    if (flag) {
...
/* 
 * Purpose:
 *   Require exactly 1 of the following 4 flags:
 *     BCM_FCOE_INTF_VSAN_SET
 *     BCM_FCOE_INTF_VSAN_INTERNAL
 *     BCM_FCOE_INTF_VSAN_INNER_VLAN
 *     BCM_FCOE_INTF_VSAN_OUTER_VLAN
 *
 * Parameters: 
 *   flags         - (IN)  flags given by the application
 *   fabric_id_sel - (OUT) the option that was chosen
 *
 * Returns:
 *   BCM_E_XXX
 */
STATIC int
_bcm_td2_id_sel_from_flags(uint32 flags, int *fabric_id_sel)
{
    if (flags & BCM_FCOE_INTF_VSAN_SET) {
        CHECK_FLAGS_NOT_SET(flags, BCM_FCOE_INTF_VSAN_INTERNAL 
                                   | BCM_FCOE_INTF_VSAN_INNER_VLAN 
                                   | BCM_FCOE_INTF_VSAN_OUTER_VLAN);
        *fabric_id_sel = FCOE_FABRIC_ID_SEL_SET;
    } else if (flags & BCM_FCOE_INTF_VSAN_INTERNAL) {
        CHECK_FLAGS_NOT_SET(flags, BCM_FCOE_INTF_VSAN_SET 
                                   | BCM_FCOE_INTF_VSAN_INNER_VLAN 
                                   | BCM_FCOE_INTF_VSAN_OUTER_VLAN);
        *fabric_id_sel = FCOE_FABRIC_ID_SEL_INTERNAL;
    } else if (flags & BCM_FCOE_INTF_VSAN_INNER_VLAN) {
        CHECK_FLAGS_NOT_SET(flags, BCM_FCOE_INTF_VSAN_SET 
                                   | BCM_FCOE_INTF_VSAN_INTERNAL 
                                   | BCM_FCOE_INTF_VSAN_OUTER_VLAN);
        *fabric_id_sel = FCOE_FABRIC_ID_SEL_INNER_VLAN;
    } else if (flags & BCM_FCOE_INTF_VSAN_OUTER_VLAN) {
        CHECK_FLAGS_NOT_SET(flags, BCM_FCOE_INTF_VSAN_SET 
                                   | BCM_FCOE_INTF_VSAN_INTERNAL 
                                   | BCM_FCOE_INTF_VSAN_INNER_VLAN);
 ...
 * Purpose:
 *   Require exactly 1 of the following 3 flags:
 *     BCM_FCOE_INTF_VSAN_PRI_SET
 *     BCM_FCOE_INTF_VSAN_PRI_INTERNAL
 *     BCM_FCOE_INTF_VSAN_PRI_REMARK
 *
 * Parameters: 
 *   flags          - (IN)  flags given by the application
 *   fabric_pri_sel - (OUT) the option that was chosen
 *
 * Returns:
 *   BCM_E_XXX
 */
STATIC int
_bcm_td2_pri_sel_from_flags(uint32 flags, int *fabric_pri_sel)
{
    if (flags & BCM_FCOE_INTF_VSAN_PRI_SET) { 
        CHECK_FLAGS_NOT_SET(flags, BCM_FCOE_INTF_VSAN_PRI_INTERNAL
                                   | BCM_FCOE_INTF_VSAN_PRI_REMARK);
        *fabric_pri_sel = FCOE_FABRIC_PRI_SEL_SET;
    } else if (flags & BCM_FCOE_INTF_VSAN_PRI_INTERNAL) {
        CHECK_FLAGS_NOT_SET(flags, BCM_FCOE_INTF_VSAN_PRI_SET
                                   | BCM_FCOE_INTF_VSAN_PRI_REMARK);
        *fabric_pri_sel = FCOE_FABRIC_PRI_SEL_INTERNAL;
    } else if (flags & BCM_FCOE_INTF_VSAN_PRI_REMARK) {
        CHECK_FLAGS_NOT_SET(flags, BCM_FCOE_INTF_VSAN_PRI_SET
                                   | BCM_FCOE_INTF_VSAN_PRI_INTERNAL);
                             ...
                             bcm_fcoe_intf_config_t *cfg)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_td2_vft_action_from_flags(cfg->flags, &vft_action));
    BCM_IF_ERROR_RETURN(_bcm_td2_pri_sel_from_flags(cfg->flags, &fabric_pri_sel));
    BCM_IF_ERROR_RETURN(_bcm_td2_id_sel_from_flags(cfg->flags, &fabric_id_sel));
                             ...
                             bcm_fcoe_intf_config_t *cfg)
{
    ...
    switch (soc_mem_field32_get(unit, mem, buf, FCOE_FABRIC_ID_SELf)) {
        case FCOE_FABRIC_ID_SEL_SET:
            cfg->flags |= BCM_FCOE_INTF_VSAN_SET;
            break;

        case FCOE_FABRIC_ID_SEL_INTERNAL:
            cfg->flags |= BCM_FCOE_INTF_VSAN_INTERNAL;
            break;

        case FCOE_FABRIC_ID_SEL_INNER_VLAN:
            cfg->flags |= BCM_FCOE_INTF_VSAN_INNER_VLAN;
            break;

        case FCOE_FABRIC_ID_SEL_OUTER_VLAN:
            cfg->flags |= BCM_FCOE_INTF_VSAN_OUTER_VLAN;
            break;
    ...
    switch (soc_mem_field32_get(unit, mem, buf, FCOE_FABRIC_PRI_SELf)) {
        case FCOE_FABRIC_PRI_SEL_SET:
            cfg->flags |= BCM_FCOE_INTF_VSAN_PRI_SET;
            break;

        case FCOE_FABRIC_PRI_SEL_INTERNAL:
            cfg->flags |= BCM_FCOE_INTF_VSAN_PRI_INTERNAL;
            break;

        case FCOE_FABRIC_PRI_SEL_REMARK:
            cfg->flags |= BCM_FCOE_INTF_VSAN_PRI_REMARK;
            break;
                                ...
                                bcm_fcoe_vsan_translate_key_config_t *key)
{
    if (soc_feature(unit, soc_feature_fcoe)) {
        ...
        /* common to all 3 key types */
        soc_VLAN_XLATEm_field32_set(unit, vent, XLATE__VSAN_IDf, key->vsan);
        ...
        /* SGLP - Use SGLP {trunk, ModId(7,0), Port(6,0)} as the Source Field*/
        ...
        if (BCM_GPORT_IS_TRUNK(key->port)) {
            ...
            soc_VLAN_XLATEm_field32_set(unit, vent, XLATE__TGIDf, trunk_out);
        } else {
