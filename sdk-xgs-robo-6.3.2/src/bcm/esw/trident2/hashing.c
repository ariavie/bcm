/*
 * $Id: hashing.c 1.2.2.2 Broadcom SDK $
...
STATIC uint16
_bcm_td2_hash_crc16(uint32 hash_words[7], int n)
{
    uint8 data[30];
    int i;
    uint16 crc;
...
STATIC uint16
_shr_td2_crc16_ccitt(int crc, unsigned char *data, int len)
{
    int         i;
    int             j;
    uint32          accum;
...
STATIC uint16
_bcm_td2_hash_crc16_ccitt(uint32 hash_words[7], int n)
{
    uint8 data[30];
    int i;
    uint16 crc;
    ...
    crc = _shr_td2_crc16_ccitt(0, data, n);
 ...
 */
static uint32
_shr_td2_crc32_castagnoli(unsigned int crc, unsigned char *data, int len)
{
    int         i;
...
STATIC uint32
_bcm_td2_hash_crc32_castagnoli(uint32 hash_words[7], int n)
{
    uint8 data[30];
    int i;
    uint32 crc;
    ...
    crc = _shr_td2_crc32_castagnoli(0, data, n);
                       ...
                       bcm_rtag7_base_hash_t *hash_res)
{
    ...
    if ((rtag7_a_sel == RTAG7_IPV6) || (rtag7_a_sel == RTAG7_IPV4)) {
        ...
        } else { hash_element_valid_bits &= ~0xc00;
        }
        if (dip_valid) {
    ...
    hash_crc16_bisync = _bcm_td2_hash_crc16(hash_word, 28);
    hash_crc16_ccitt = _bcm_td2_hash_crc16_ccitt(hash_word, 28);
    hash_crc32 = _bcm_td2_hash_crc32_castagnoli(hash_word, 28);
 ...
 */
STATIC int
select_td2_hash_subfield(int hash_sub_sel, uint32 *selected_subfield,
 ...
 *     if flow hash is not used, get SUB_SEL_LBID_UCf and 
 *        OFFSET_LBID_UCf from RTAG7_PORT_BASED_HASHm 
 * Parameters:
 *          unit - StrataSwitch PCI device unit number (driver internal).
...
main_td2_compute_lbid(int unit, bcm_rtag7_base_hash_t *hash_Base)
{
    ...
    /*register var declaration*/
    int lbid_rtag = 0;
    uint8 rtag7_lb_use_macro_flow_hash;
                  ...
                  uint32 *hash_val)
{
    ...
    BCM_VERB(("Unit %d - ecmp hash_seb_sel=%d, hash_offset=%d\n",
                           unit, hash_sub_sel, hash_offset));
    BCM_IF_ERROR_RETURN
        (select_td2_hash_subfield(hash_sub_sel, &hash_subfield, hash_Base));

    /* Barrel shift the hash subfield, then take the LSB 16 bits */
    *hash_val = ((hash_subfield >> hash_offset) | (hash_subfield << (16 - hash_offset))) & 0xffff;  
                  ...
                  uint32 *hash_val)
{
    ...
    if (rtag7_ecmp_use_macro_flow_hash) {
        ...
        /* For future designs this logic can be placed in RSEL1 */
        hash_sub_sel = soc_RTAG7_FLOW_BASED_HASHm_field32_get(unit, 
                                           &flow_based_hash_entry, SUB_SEL_RH_ECMPf);
        hash_offset  = soc_RTAG7_FLOW_BASED_HASHm_field32_get(unit, 
                                           &flow_based_hash_entry, OFFSET_RH_ECMPf);
    ...
    BCM_VERB(("Unit %d - ecmp rh hash_seb_sel=%d, hash_offset=%d\n",
                  unit, hash_sub_sel, hash_offset));
                         ...
                         uint32 *hash_value)
{
    ...
    BCM_VERB(("Unit %d - Trunk hash_sub_sel=%d, hash_offset=%d, offset_shift=%d\n",
                unit, hash_sub_sel, hash_offset, offset_shift));
    ...
    *hash_value = ((hash_subfield >> hash_offset) | (hash_subfield << (16 - hash_offset))) & offset_shift; 
                         ...
                         uint32 *hash_value)
{
    ...
    BCM_VERB(("Unit %d - Trunk RH hash_sub_sel=%d, hash_offset=%d, offset_shift=%d\n",
                unit, hash_sub_sel, hash_offset, offset_shift));
...
uint8 check_td2_ether_type_eligibility_for_rh(int unit, uint8 ecmp, uint32 rh_ethertype) 
{
...
int perform_td2_rh(int unit,uint32 flow_set_base, uint8 flow_set_size, uint8 rh_for_ecmp, 
                uint32  ecmp_hash,uint32 lag_hash, 
                uint32* resolved_member, uint8* resolved_member_is_valid) 
{
    ...
    if (rh_for_ecmp) {
        /* RTAG7 HASH FOR RH ECMP */ 
        rtag7_hash = ecmp_hash; 
    } else {

        /* RTAG7 HASH FOR RH LAG */
        rtag7_hash = lag_hash;
    }
    ...
    if (rh_for_ecmp) {
        ...
        SOC_IF_ERROR_RETURN(READ_RH_ECMP_FLOWSETm(unit,
                    MEM_BLOCK_ANY, flow_set_index, &ethertype_entry));
            *resolved_member_is_valid = soc_RH_ECMP_ETHERTYPE_ELIGIBILITY_MAPm_field32_get(
                                                        unit, &ethertype_entry, VALIDf);
            *resolved_member = soc_RH_ECMP_ETHERTYPE_ELIGIBILITY_MAPm_field32_get(unit,
                    &ethertype_entry, NEXT_HOP_INDEXf);
    } else {
        /* Read RH_LAG_FLOWSET TABLE */
        SOC_IF_ERROR_RETURN(READ_RH_ECMP_FLOWSETm(unit,
                    MEM_BLOCK_ANY, flow_set_index, &ethertype_entry));
            *resolved_member_is_valid = soc_RH_LAG_ETHERTYPE_ELIGIBILITY_MAPm_field32_get(
                                                        unit, &ethertype_entry, VALIDf);
            
            BCM_VERB(("Unit %d:RH_LAG_ETHERTYPE_ELIGIBILITY_MAP.PORT_NUMf=%d\n",
                    unit,soc_RH_LAG_ETHERTYPE_ELIGIBILITY_MAPm_field32_get(unit,  &ethertype_entry, PORT_NUMf)));
            BCM_VERB(("Unit %d:RH_LAG_ETHERTYPE_ELIGIBILITY_MAP.MODULE_IDF=%d\n",
                    unit,soc_RH_LAG_ETHERTYPE_ELIGIBILITY_MAPm_field32_get(unit,  &ethertype_entry, MODULE_IDf)));


            *resolved_member = (soc_RH_LAG_ETHERTYPE_ELIGIBILITY_MAPm_field32_get(unit,
                    &ethertype_entry, PORT_NUMf), 
                    soc_RH_LAG_ETHERTYPE_ELIGIBILITY_MAPm_field32_get(unit,
                    &ethertype_entry, MODULE_IDf));
             BCM_VERB(("Unit %d: rh ecmp flowset *resolved_member=%d\n",unit, *resolved_member));
...
check_td2_ecmp_rh_enable (int unit, int ecmp_group, uint32 ethertype) 
{
    ...
    rh_ecmp_ethertype_enable = check_td2_ether_type_eligibility_for_rh(unit, 1, ethertype);
                  ...
                  uint8 ecmp_rh_enable, uint32 ethertype)
{
    ...
    uint32 resolved_ecmp_member;
    uint8  resolved_ecmp_member_valid;
    ...
    if (ecmp_rh_enable) {
        ...
        perform_td2_rh(unit, ecmp_rh_flow_set_base, ecmp_rh_flow_set_size, 1,hash_rh_index,0,
                  &resolved_ecmp_member, &resolved_ecmp_member_valid);

        if (resolved_ecmp_member_valid) {
            *nh_id = resolved_ecmp_member & 0xffff; 
        } else {
...
check_td2_lag_rh_enable (int unit, int tgid, uint32 ethertype) 
{
    ...
    rh_lag_ethertype_enable = check_td2_ether_type_eligibility_for_rh(unit, 0, ethertype);
...
get_td2_hash_trunk(int unit, int tgid, uint32 hash_index, 
                bcm_gport_t *dst_tgid, uint32 lag_rh_hash, 
                  uint8 lag_rh_enable, uint32 ethertype)
{
    ...
    uint32 resolved_lag_member;
    uint8  resolved_lag_member_valid;
    ...
    if (lag_rh_enable) {
        ...
        perform_td2_rh(unit, lag_rh_flow_set_base, lag_rh_flow_set_size, 0, 0, lag_rh_hash,
                   &resolved_lag_member, &resolved_lag_member_valid);

        if (resolved_lag_member_valid) {
            mod_id = resolved_lag_member & 0x7f;
            port_id = (resolved_lag_member >> 7 ) & 0xff;
                                   ...
                                   bcm_if_t *dst_intf)
{
    ...
    uint8           lag_rh_enable;
    bcm_ethertype_t ethertype = pkt_info->ethertype;
    int             rc;
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_TRUNK)) {
        ...
        if (0 != (check_td2_lag_rh_enable(unit, BCM_GPORT_TRUNK_GET(pkt_info->trunk_gport), 
                  ethertype ))) { 
             lag_rh_enable = TRUE;
             rc = 
        ...
        BCM_IF_ERROR_RETURN
            (get_td2_hash_trunk(unit, BCM_GPORT_TRUNK_GET(pkt_info->trunk_gport),
