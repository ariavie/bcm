/*
 * $Id: trill.c 1.173.2.1 Broadcom SDK $
 *
...
_bcm_td_trill_adjacency_set(int unit, int nh_index)
{
   ...
   soc_mem_mac_addr_get(unit, EGR_L3_NEXT_HOPm, &egr_nh, 
    ...
    for (idx = 0; idx < num_local_ports; idx++) {
         ...
         BCM_IF_ERROR_RETURN(
              bcm_td_metro_myStation_add(unit, src_mac_addr, vid, local_ports[idx]));
...
_bcm_td_trill_adjacency_reset(int unit, int nh_index)
{
    ...
    for (idx = 0; idx < num_local_ports; idx++) {
         ...
         rv = bcm_td_metro_myStation_delete(unit, src_mac_addr, vid, local_ports[idx]);
...
{ 
   ...
   int rv=BCM_E_NONE;
    ...
    if (BCM_FAILURE(rv)) {
     ...
     /* Obtain nh_index for trunk_id - Only for Trill, L2GRE, VXLAN */
   ...
   int rv=BCM_E_NONE;
    ...
    for (idx = 0; idx < trunk_member_count; idx++) {
        ...
        if (0 != old_nh_index) {
            ...
            /* Delete MY_STATION entry */
            rv = bcm_td_metro_myStation_delete(unit, src_mac_addr, vid, trunk_member_array[idx]);
            if ((rv != BCM_E_NOT_FOUND) || (rv != BCM_E_FULL)) {
                if (rv != BCM_E_NONE) {
                   return rv;
                 }
...
bcm_td_trill_port_add(int unit, bcm_trill_port_t *trill_port)
{
    ...
    int ref_count = 0;
    int  vp, num_vp=0, base_idx=0;
    int  idx=0, max_ent_count=0;
    source_vp_entry_t svp;
    ing_dvp_table_entry_t dvp;
    uint32 hw_buf[SOC_MAX_MEM_FIELD_WORDS]; /* Buffer to read hw entry. */
    uint32 flags=0;
       ...
       if (!soc_mem_field32_get(unit, VLAN_TABm, vtab, 
           ...
           for (i = 0; i < 3; i++) {
               ...
               mc_idx = soc_mem_field32_get(unit, VLAN_TABm, vtab, 
                                            group_type[i]);
               if (BCM_FAILURE(_bcm_tr_multicast_ipmc_group_type_get(unit, mc_idx, &mc_grp))) {
                   continue;
               }
