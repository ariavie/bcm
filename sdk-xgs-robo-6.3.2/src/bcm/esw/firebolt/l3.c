/*
 * $Id: l3.c 1.866.2.13 Broadcom SDK $
...
STATIC int _bcm_xgs3_ecmp_max_grp_size_set(int unit, int grp_size);
#ifdef BCM_WARM_BOOT_SUPPORT
        ...
        int32 *hostmode, int32 *intfMapmode)
{
    ...
    *intfMapmode = 0;
    ...
    if (stable_size > 0) { /* Limited and Extended recovery only */
         ...
         */
        soc_scache_ptr_get(unit, scache_handle, &l3_scache_ptr, &recovered_sz);
        recovered_sz -= SOC_WB_SCACHE_CONTROL_SIZE;
        l3_ip4_options_recover = (recovered_sz == alloc_sz) ? 1 : 0;
#endif /* BCM_TRIDENT2_SUPPORT */
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            if (soc_feature(unit, soc_feature_l3_ip4_options_profile)) {
                if ((recovered_ver >= BCM_WB_VERSION_1_6) ||
                    ...
                    additional_scache_sz += (alloc_sz - recovered_sz);
                }
            }
#endif /* BCM_TRIDENT2_SUPPORT */
...
bcm_xgs3_max_ecmp_set(int unit, int max_paths)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_xgs3_ecmp_max_grp_size_set(unit, max_paths));
 ...
 */
int
bcm_fb_lpm_ent_init(int unit, _bcm_defip_cfg_t *lpm_cfg,
                     defip_entry_t *lpm_entry)
{
    bcm_ip_t ip4_mask;
    int vrf_id;
...
_bcm_fb_lpm_get(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx)
{
    ...
    /* Initialize lkup key. */
    BCM_IF_ERROR_RETURN(bcm_fb_lpm_ent_init(unit, lpm_cfg, &lpm_key));
...
_bcm_fb_lpm_add(int unit, _bcm_defip_cfg_t *lpm_cfg, int nh_ecmp_idx)
{
     ...
     */
    BCM_IF_ERROR_RETURN(bcm_fb_lpm_ent_init(unit, lpm_cfg, &lpm_entry));
...
_bcm_fbx_lpm_get(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx)
{
    ...
    switch (mem) {
      ...
      case EXT_IPV4_DEFIPm:
        ...
        case L3_DEFIP_128m:
#if defined(BCM_KATANA_SUPPORT)
            if (SOC_IS_KATANAX(unit) && (lpm_ipv6_info->ipv6_128b.depth > 0)) {
