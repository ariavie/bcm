/*
 * $Id: port.c 1.53.2.1 Broadcom SDK $
...
_bcm_trx_lport_tab_default_entry_add(int unit, soc_profile_mem_t *prof) 
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, RTAG7_HASH_CFG_SEL_TRILL_ECMPf)) {
         soc_LPORT_TABm_field32_set(unit, &buf, RTAG7_HASH_CFG_SEL_TRILL_ECMPf,
                                    1);
    }
    ...
    entry[0] = &buf;
    rv = soc_profile_mem_add(unit, prof, entry, 1, &lport_index);
    ...
    SOC_PROFILE_MEM_REFERENCE(unit, prof, lport_index,
                              stm_num_entries);
