/*
 * $Id: l3.c 1.132.2.2 Broadcom SDK $
...
_bcm_tr3_l3_ecmp_grp_add(int unit, int ecmp_grp, void *buf, int max_paths)
{
    ...
    if (BCM_XGS3_L3_ENT_REF_CNT(BCM_XGS3_L3_TBL_PTR(unit,
        ...
        if (BCM_SUCCESS(rv) &&
            ((data.entry_index + max_paths - 1) <
              BCM_XGS3_L3_ECMP_TBL_SIZE(unit) - 1)) {
            ecmp_idx = data.entry_index;
            BCM_XGS3_L3_ENT_REF_CNT_INC(data.tbl_ptr, data.entry_index,
                                        max_paths);
            ecmp_table_incr = 1;
    ...
    if ((ecmp_idx + max_paths - 1) >= BCM_XGS3_L3_ECMP_TBL_SIZE(unit)) {
