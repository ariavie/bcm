/*
 * $Id: mim.c 1.147.2.2 Broadcom SDK $
...
bcm_tr2_mim_enable(int unit, int enable)
{
    ...
    pbmp_t loopback_pbmp;
    multipass_loopback_bitmap_entry_t mim_loopback;
    ...
    if (SOC_IS_TD_TT(unit)) {
        sal_memset(&mim_loopback, 0, 
                   sizeof(multipass_loopback_bitmap_entry_t));
        if (enable) {
            SOC_PBMP_ASSIGN(loopback_pbmp, PBMP_LB(unit));
        } else {
            SOC_PBMP_CLEAR(loopback_pbmp);
        }
        soc_mem_pbmp_field_set(unit, MULTIPASS_LOOPBACK_BITMAPm, 
                               &mim_loopback, BITMAPf, &loopback_pbmp);
        BCM_IF_ERROR_RETURN
            (soc_mem_write(unit, MULTIPASS_LOOPBACK_BITMAPm,
                           MEM_BLOCK_ALL, 0, &mim_loopback));
    }
#endif /* BCM_TRIDENT_SUPPORT */ 
...
bcm_tr2_mim_vpn_create(int unit, bcm_mim_vpn_config_t *info)
{
    ...
    if (info->flags & BCM_MIM_VPN_WITH_ID) {
            ...
            if ( info->vpn < mim_vpn_min || info->vpn > (mim_vpn_min+num_vfi-1) ) {
                MIM_UNLOCK(unit);
                return BCM_E_PARAM;
            }
    ...
    if ( info->lookup_id >= 0 ) {
        ...
        if ((info->flags & BCM_MIM_VPN_WITH_ID) &&
            ...
            soc_MPLS_ENTRYm_field32_set(unit, &mpls_entry, MIM_ISID__ISIDf, old_info.lookup_id);   
            rv = soc_mem_delete(unit, MPLS_ENTRYm, MEM_BLOCK_ALL, &mpls_entry);
            if (BCM_FAILURE(rv)) {
                MIM_UNLOCK(unit);
                return rv;
            } 
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) ||
        ...
        if (BCM_GPORT_IS_SET(mim_port->port) &&
            _BCM_KT2_GPORT_IS_LINKPHY_OR_SUBTAG_SUBPORT_GPORT(unit,
                mim_port->port) && (mod_out == (my_modid + 1))) {
