/*
 * $Id: bst.c 1.36.2.1 Broadcom SDK $
...
typedef int (*_bcm_bst_hw_trigger_set)(int unit, _bcm_bst_resource_t res);
typedef int (*_bcm_bst_hw_trigger_reset)(int unit, _bcm_bst_resource_t res);
...
STATIC int _bcm_bst_sync_sw_stat_get(int unit, int res, int port, int index);
...
_bcm_td2_bst_control_get(int unit, bcm_switch_control_t type, int *arg)
{
    ...
    case bcmSwitchBstTrackingMode:
        BCM_IF_ERROR_RETURN(READ_MISCCONFIGr(unit, &rval));
        *arg = soc_reg_field_get(unit, MISCCONFIGr, rval, 
                                 BST_TRACKING_MODEf);
        break;
    default:
        return BCM_E_UNAVAIL;
...
_bcm_bst_td2_intr_to_resources(int unit, uint32 *flags)
{
    ...
    uint64 rval_uc, rval_db, rval_mcqe, rval64;
    int index[2] = { -1, -1 };
    int res = -1, pipe = 0;
    ...
    if (index[0] != -1) {
        _bcm_bst_sync_hw_trigger(unit, res, -1, index[0]);
    } else {
        _bcm_bst_sync_hw_trigger(unit, res, -1, index[1]);
    }
...
STATIC int 
_bcm_bst_td2_hw_trigger_set(int unit, _bcm_bst_resource_t res)
{
    uint32 rval;
...
STATIC int 
_bcm_bst_td2_hw_trigger_reset(int unit, _bcm_bst_resource_t res)
{
    uint32 rval;
    _bcm_bst_resource_info_t *resInfo;
    soc_field_t fld[2] = { INVALIDf, INVALIDf };
...
_bcm_bst_td2_init(int unit)
{
    ...
    pres->index_min = 0;
    pres->index_max = 2 * soc_mem_index_count(unit, MMU_THDU_XPIPE_BST_QUEUEm);
...
STATIC int
_bcm_bst_check_intr_status(int unit, _bcm_bst_resource_t res)
{
                            ...
                            uint32 *p_profile)
{
    ...
    if (bst_info->hw_trigger_set && cells) {
        bst_info->hw_trigger_set(unit, res);
    }
                        ...
                        uint32 flags, uint64 *pstat)
{
    ...
    sal_mutex_take(bst_info->bst_reslock, sal_mutex_FOREVER);
    _bcm_bst_sync_sw_stat_get(unit, res, -1, start_hw_index);
    sal_mutex_give(bst_info->bst_reslock);

    COMPILER_64_SET(*pstat, 0, resInfo->p_stat[start_hw_index]);

#if defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT)
    if (_bcm_bst_check_intr_status(unit, res)) {
        _bst_intr_enable_set(unit, 1);
                           ...
                           void *user_data)
{
    ...
    for (fi = 0; fi < 32; fi++) {
        ...
        do {
            ...
            sal_mutex_take(bst_info->bst_reslock, sal_mutex_FOREVER);
            _bcm_bst_sync_sw_stat_get(unit, res, -1, -1);
            sal_mutex_give(bst_info->bst_reslock);

            for (ii = start_hw_index; ii <= end_hw_index; ii++) {
                COMPILER_64_SET(sval, 0, resInfo->p_stat[ii]);
    ...
    for (pipe = 0; pipe < _BCM_MAX_PIPES; pipe++) {
        if ((mem = resInfo->stat_mem[pipe]) != INVALIDm) {
            for (idx = min_hw_idx; idx <= max_hw_idx; idx++) {
                SOC_IF_ERROR_RETURN(soc_mem_read(unit, mem, MEM_BLOCK_ALL, idx, entry));
            
                resInfo->p_stat[idx + pipe_offset] = soc_mem_field32_get(unit, mem, 
                                                             entry, resInfo->stat_field);
                soc_mem_field32_set(unit, mem, entry, resInfo->stat_field, 0);

                SOC_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL, idx, entry));
            }
...
STATIC int
_bcm_bst_sync_sw_stat_get(int unit, int res, int port, int index)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    int rv, pipe;
