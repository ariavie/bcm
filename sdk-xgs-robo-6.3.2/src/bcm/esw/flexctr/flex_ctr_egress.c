/*
 * $Id: flex_ctr_egress.c 1.8 Broadcom SDK $
    ...
    uint32                                      *total_counters)
{
    ...
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        ...
        pkt_attr_bits.cng = egr_pkt_attr_bits_g.cng;
        pkt_attr_bits.cng_mask = egr_pkt_attr_bits_g.cng_mask;
        pkt_attr_bits.cng_pos = egr_pkt_attr_bits_g.cng_pos;
        total_egress_bits +=pkt_attr_bits.cng;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_INT_PRI_ATTR_BITS) {
        pkt_attr_bits.int_pri = egr_pkt_attr_bits_g.int_pri ;
        pkt_attr_bits.int_pri_mask = egr_pkt_attr_bits_g.int_pri_mask;
        pkt_attr_bits.int_pri_pos = egr_pkt_attr_bits_g.int_pri_pos;
        total_egress_bits +=pkt_attr_bits.int_pri;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_VLAN_FORMAT_ATTR_BITS) {
        pkt_attr_bits.vlan_format = egr_pkt_attr_bits_g.vlan_format ;
        pkt_attr_bits.vlan_format_mask= egr_pkt_attr_bits_g.vlan_format_mask;
        pkt_attr_bits.vlan_format_pos= egr_pkt_attr_bits_g.vlan_format_pos;
        total_egress_bits +=pkt_attr_bits.vlan_format;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_OUTER_DOT1P_ATTR_BITS) {
        pkt_attr_bits.outer_dot1p = egr_pkt_attr_bits_g.outer_dot1p;
        pkt_attr_bits.outer_dot1p_mask= egr_pkt_attr_bits_g.outer_dot1p_mask;
        pkt_attr_bits.outer_dot1p_pos= egr_pkt_attr_bits_g.outer_dot1p_pos;
        total_egress_bits +=pkt_attr_bits.outer_dot1p;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_INNER_DOT1P_ATTR_BITS) {
        pkt_attr_bits.inner_dot1p = egr_pkt_attr_bits_g.inner_dot1p ;
        pkt_attr_bits.inner_dot1p_mask = egr_pkt_attr_bits_g.inner_dot1p_mask;
        pkt_attr_bits.inner_dot1p_pos = egr_pkt_attr_bits_g.inner_dot1p_pos;
        total_egress_bits +=pkt_attr_bits.inner_dot1p;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_EGRESS_PORT_ATTR_BITS) {
        pkt_attr_bits.egr_port = egr_pkt_attr_bits_g.egr_port ;
        pkt_attr_bits.egr_port_mask = egr_pkt_attr_bits_g.egr_port_mask;
        pkt_attr_bits.egr_port_pos = egr_pkt_attr_bits_g.egr_port_pos;
        total_egress_bits +=pkt_attr_bits.egr_port;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_TOS_ATTR_BITS) {
        pkt_attr_bits.tos = egr_pkt_attr_bits_g.tos;
        pkt_attr_bits.tos_mask = egr_pkt_attr_bits_g.tos_mask;
        pkt_attr_bits.tos_pos = egr_pkt_attr_bits_g.tos_pos;
        total_egress_bits +=pkt_attr_bits.tos;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS) {
        pkt_attr_bits.pkt_resolution = egr_pkt_attr_bits_g.pkt_resolution ;
        pkt_attr_bits.pkt_resolution_mask= egr_pkt_attr_bits_g.
                                           pkt_resolution_mask;
        pkt_attr_bits.pkt_resolution_pos= egr_pkt_attr_bits_g.
                                           pkt_resolution_pos;
        total_egress_bits +=pkt_attr_bits.pkt_resolution;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_SVP_TYPE_ATTR_BITS) {
        pkt_attr_bits.svp_type = egr_pkt_attr_bits_g.svp_type ;
        pkt_attr_bits.svp_type_mask = egr_pkt_attr_bits_g.svp_type_mask ;
        pkt_attr_bits.svp_type_pos = egr_pkt_attr_bits_g.svp_type_pos ;
        total_egress_bits +=pkt_attr_bits.svp_type;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_DVP_TYPE_ATTR_BITS) {
        pkt_attr_bits.dvp_type = egr_pkt_attr_bits_g.dvp_type ;
        pkt_attr_bits.dvp_type_mask = egr_pkt_attr_bits_g.dvp_type_mask;
        pkt_attr_bits.dvp_type_pos = egr_pkt_attr_bits_g.dvp_type_pos;
        total_egress_bits += pkt_attr_bits.dvp_type;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_DROP_ATTR_BITS) {
        pkt_attr_bits.drop = egr_pkt_attr_bits_g.drop;
        pkt_attr_bits.drop_mask = egr_pkt_attr_bits_g.drop_mask;
        pkt_attr_bits.drop_pos = egr_pkt_attr_bits_g.drop_pos;
        total_egress_bits +=pkt_attr_bits.drop;
    }
    if (uncmprsd_attr_selectors->uncmprsd_attr_bits_selector &
        BCM_STAT_FLEX_EGR_UNCOMPRESSED_USE_IP_PKT_ATTR_BITS) {
        pkt_attr_bits.ip_pkt = egr_pkt_attr_bits_g.ip_pkt ;
        pkt_attr_bits.ip_pkt_mask = egr_pkt_attr_bits_g.ip_pkt_mask ;
        pkt_attr_bits.ip_pkt_pos = egr_pkt_attr_bits_g.ip_pkt_pos ;
    ...
    uint32                                    *total_counters)
{
    ...
    if (cmprsd_attr_selectors->pkt_attr_bits.vlan_format !=0) {
        if (cmprsd_attr_selectors->pkt_attr_bits.vlan_format >
            egr_pkt_attr_bits_g.vlan_format) {
            return BCM_E_PARAM;
        }
    ...
    if (cmprsd_attr_selectors->pkt_attr_bits.outer_dot1p !=0) {
        if (cmprsd_attr_selectors->pkt_attr_bits.outer_dot1p  > 
            egr_pkt_attr_bits_g.outer_dot1p) {
            return BCM_E_PARAM;
        }
        if (pkt_pri_map_used==1) {
            return BCM_E_PARAM;
    ...
    if (cmprsd_attr_selectors->pkt_attr_bits.inner_dot1p !=0) {
        if (cmprsd_attr_selectors->pkt_attr_bits.inner_dot1p  >
            egr_pkt_attr_bits_g.inner_dot1p) {
            return BCM_E_PARAM;
        }
        if (pkt_pri_map_used==1) {
            return BCM_E_PARAM;
    ...
    if (cmprsd_attr_selectors->pkt_attr_bits.pkt_resolution !=0) {
        if (cmprsd_attr_selectors->pkt_attr_bits.pkt_resolution  > 
            egr_pkt_attr_bits_g.pkt_resolution) {
            return BCM_E_PARAM;
        }
