            ...
            uint8 addr_len, uint8 *buf, uint8 buf_len)
{
    ...
    if (platform_info == NULL) {
        printk("Platform HAL not initialized \n");
        return -1;
    }
    if (platform_info->f_i2c_op == NULL) {
        printk("Platform HAL does not support i2c operations \n");
        return -1;
    }

    if (buf == NULL) {
        printk("Input param - buf is invalid (NULL) \n");
        return -1;
    }
    switch (addr_len) {
    case 0:
    ...
    case 4:
        ...
        break; 
    default:
        printk("Address length:%d not supported \n", addr_len);
        return -1;
    }
    ...
    case 4:
        ...
        break; 
    default:
        printk("Data buffer length:%d not supported \n", buf_len);
        return -1;
    }
...
sal_i2c_config_set(int unit, uint32 flags)
{
    ...
    if (flags & (~SAL_I2C_FAST_ACCESS)) {
        printk("%s: Unsupported flags specified: %x", FUNCTION_NAME(),
...
sal_i2c_config_get(int unit, uint32 *flags)
{
    ...
    if (flags == NULL) {
        printk("Invalid flags (NULL) param. \n");
        return -1;
    }
