/*
 * $Id: bcm-knet.c 1.86.2.1 Broadcom SDK $
...
#define POLLS_PER_SEC 10
#define POLL_INTERVAL (HZ / POLLS_PER_SEC)

#define RXTICKS_PER_SEC 10
#define RXTICK_INTERVAL (HZ / RXTICKS_PER_SEC)
...
typedef struct bkn_switch_info_s {
    ...
    struct {
        ...
        unsigned long pkts;     /* Packet counter */
        unsigned long pkts_ref; /* Packet count for packet rate calculation */
    } rx[NUM_RX_CHAN];
} bkn_switch_info_t;
...
xgsm_dma_chan_abort(bkn_switch_info_t *sinfo, int chan, int polls)
{
    ...
    DEV_WRITE32(sinfo, CMICM_DMA_CTRLr + 4 * chan, ctrl | 0x4); 
    ...
    for (p = 0; p < polls; p++) {
        ...
        if (!(dma_stat & DS_CMC_DMA_ACTIVE(chan))) {
            /* Restore previous control value */            
            DEV_WRITE32(sinfo, CMICM_DMA_CTRLr, ctrl); 
...
bkn_rx_refill(bkn_switch_info_t *sinfo, int chan)
{
    ...
    while (sinfo->rx[chan].free < MAX_RX_DCBS && sinfo->rx[chan].tokens > 0) {
...
{
    bkn_switch_info_t *sinfo = (bkn_switch_info_t *)context;
    unsigned long cur_jif, pkt_diff, ticks;
    int chan;

    sinfo->rxtick.expires = jiffies + RXTICK_INTERVAL;

    /* For debug purposes we maintain a rough actual packet rate */
    if (++sinfo->rxticks >= RXTICKS_PER_SEC) {
        for (chan = 0; chan < NUM_RX_CHAN; chan++) {
...
bkn_create_sinfo(int dev_no)
{
    ...
    for (chan = 0; chan < NUM_RX_CHAN; chan++) {
        ...
        if (sinfo->rx[chan].rate_max > 0) {
            ...
            sinfo->rx[chan].burst_max = sinfo->rx[chan].rate_max / 2;
            sinfo->rx[chan].tokens = sinfo->rx[chan].burst_max;
        }
    }
    add_timer(&sinfo->rxtick);
...
bkn_do_skb_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    while (dcbs_done < budget) {
        ...
        if ((dcb[1] & (1 << 16)) == 0) {
            sinfo->rx[chan].chain_complete = 1;
            /*
             * Assume DMA is complete if we reached end of chain.
             * This allows us to restart the chain from the NAPI poll
             * function without taking a chain done interrupt.
             */
            if (sinfo->napi_poll_mode) {
                sinfo->rx[chan].running = 0;
            }
...
bkn_poll(struct net_device *dev, int *budget)
{
    ...
    for (chan = 0; chan < NUM_RX_CHAN; chan++) {
        ...
        *budget -= rx_dcbs_done;
        ...
        if (sinfo->rx[chan].all_done == 0 || sinfo->napi_poll_again) {
            all_done = 0;
        }
    }
                    ...
                    unsigned long count, void *data)
{
    ...
    if (sinfo && (ptr = strstr(rate_str, "rx_rate=")) != NULL) {
        ptr += 7;
        chan = 0;
        do {
            ptr++;
            rate = simple_strtol(ptr, NULL, 10);

            spin_lock_irqsave(&sinfo->lock, flags);

            sinfo->rx[chan].rate_max = rate;
            sinfo->rx[chan].burst_max = sinfo->rx[chan].rate_max / 2;
            sinfo->rx[chan].tokens = sinfo->rx[chan].burst_max;
