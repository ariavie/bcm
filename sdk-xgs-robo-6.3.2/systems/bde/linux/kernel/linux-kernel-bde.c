/*
 * $Id: linux-kernel-bde.c 1.407.2.3 Broadcom SDK $
...
_iproc_device_create (void) {
    ...
    bde_ctrl_t *ctrl;
    uint32 dev_rev_id;
    resource_size_t paddr = IPROC_CMICD_BASE;
    uint32 size = IPROC_CMICD_SIZE;
    sal_vaddr_t iproc_cca_base;
    uint32 cca_cid;
    uint32 has_cmic = 0;

    /* Read ChipcommonA chip id register to identify current SOC */
    iproc_cca_base = (sal_vaddr_t)IOREMAP(IPROC_CHIPCOMMONA_BASE, 0x3000);
    if (!iproc_cca_base) {
...
_cleanup(void)
{
    ...
    for (i = 0; i < _ndevices; i++) {
        ...
        if (ctrl->dev_type & BDE_AXI_DEV_TYPE) {
            release_mem_region(ctrl->phys_address, IPROC_CMICD_SIZE);
        }
    }
#ifdef BCM_ICS
#else
...
_pprint(void)
{
    ...
    for (i = 0; i < _ndevices; i++) {
        ...
        if (ctrl->dev_type & BDE_PCI_DEV_TYPE) {
            pprintf("PCI device %d:0x%x:0x%x:%d:0x%.8lx:0x%.8lx:%d%s\n",
                    i,
            ...
            pprintf("iProc Device %d:0x%x:0x%x\n",
                    i,
                    ctrl->bde_dev.device,
                    ctrl->bde_dev.rev);
...
static int _iproc_write(int d, uint32_t addr, uint32_t data);
...
static _iproc_subwin_t iproc_subwin_default[LINUX_BDE_MAX_DEVICES];
