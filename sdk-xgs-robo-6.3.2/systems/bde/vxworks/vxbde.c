/*
 * $Id: vxbde.c 1.155.32.2 Broadcom SDK $
...
_iproc_offset(int d, uint32 addr)
{
    ...
    raddr = IPROC_PAXB_IMAP0_ADDR + (IPROC_DEFAULT_SUBWIN*4);
    _iproc_write(d, raddr, (addr_page | 0x1));
       ...
       uint8 pciRevID)
{
    ...
    if (pciVenID == PLX_VENDOR_ID) {
        ...
        if (pciDevID == PCP_PCI_DEVICE_ID) {
            ...
            _n_devices++;
               ...
               ((pciDevID & 0xFFFF) != BCM88030_DEVICE_ID) &&  /* Caladan3 */
    ...
    if (iproc) {
        ...
        iproc_map[_switch_n_devices-1][0] = IPROC_CCA_PAGE;
        iproc_map[_switch_n_devices-1][1] = IPROC_CCB_PAGE;
        iproc_map[_switch_n_devices-1][2] = IPROC_PAXB_PAGE;
    }
    vxd->flags = 0x0;
    if(flags & BDE_ETHER_DEV_TYPE){
#if defined(KEYSTONE)
